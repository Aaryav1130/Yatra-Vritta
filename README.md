...
# Comprehensive Features of Yatra-Vritta
...
## Medical Tourism: AI-Driven Personalization for Patients
...
## MICE Travel: Smart Event Management for Corporate Clients
...
## Destination Weddings: Intelligent Planning and Coordination
...
# 🔍 Detailed Project Architecture & Components
...
### **🔹 Backend Core**
- **`app.js`** – The primary backend entry point that connects services.
- **`config/generateToken.js`** – Manages user authentication token generation.
- **`db.js`** – Handles database connections and queries.
- **`middleware/authMiddleware.js`** – Middleware for user authentication and access control.
- **`models/User.js`** – Defines user schema and database interactions.
- **`routes/`** – Contains API routes:
  - `MICE_routes.js` – Handles event-related queries.
  - `Medical_routes.js` – Manages medical tourism API endpoints.
  - `Wedding_routes.js` – Manages wedding planning APIs.

### **🔹 Service Layer**
- **`services/userService.js`** – Manages user-related functionalities.
- **`services_backend/`** – Contains core service scripts:
  - `medicalbackend.py` – Handles AI-powered medical tourism recommendations.
  - `mice_backend.py` – Manages MICE event organization.
  - `wedding_backend.py` – Processes wedding-related requests.

### **🔹 Sightseeing Backend**
- **Handles sightseeing queries and integrations with tourism APIs.**
- **Contains:**
  - `controllers/authcontroller.js` – Manages sightseeing authentication.
  - `server.js` – Handles sightseeing backend API requests.

### **🔹 Virtual Assistant**
- **AI-powered chatbot backend using Python.**
- **Located at `virtual assistant/backend.py`** to provide real-time travel guidance.
...
### **🔹 Core Event Features**
- **`Wedding-app/`** – Dedicated wedding planning system.
- **`app/`** – Core business logic for event creation and management.
- **`api/`** – API endpoints for:
  - **Flight booking** (`searchFlights.ts`, `bookFlight.ts`, `modifyBooking.ts`)
  - **Sightseeing reservations** (`searchSightseeing.ts`, `bookSightseeing.ts`).
  - **Payment Integrations** via Stripe and Clerk.
- **`components/`** – UI components for event planning (Filters, Forms, Checkout system).

### **🔹 Wedding Planning**
- **Includes:** 
  - Venue selection tools (`WeddingVenue.jsx`, `WeddingResort.jsx`).
  - Budget planners (`Budget.jsx`).
  - Vendor management (`WeddingVendor.jsx`, `Caterers.jsx`, `MakeupArtists.jsx`).
- **Front-end is React-based (`Wedding_Frontend/`).**
...
- **`front.py`** – Main application interface.
- **`medicalfrontend.py`** – UI for medical tourism recommendations.
- **`mice_frontend.py`** – MICE event management front-end.
- **`virtual_assistant.py`** – AI-powered chatbot front-end.
- **`wedding_frontend.py`** – UI for wedding planning.
...
- **Medical Tourism AI Model** (`models/models_medical/medical_tourism_model.pkl`).
- **MICE Event Predictions** (`models/models_mice/city_ranking_model.pkl`).
- **Wedding Destination AI Rankings** (`models/models_wedding/wedding_ranking_model.pkl`).

### **🔹 Data and Training**
- **AI-training datasets stored in** `models/dataset/`.
- **Score-based ranking** for medical, MICE, and wedding destinations (`dataset/score/`).

---

## **📂 Testing & Development**
Yatra-Vritta follows a **test-driven development (TDD) approach**, with test scripts available in `test/` and `test_services/`.

### **🔹 Test Modules**
- **Airline Booking Tests** (`test/airlines/airline_back.py`).
- **MICE Event Planning Tests** (`test/mice/backend.py`).
- **Medical Tourism Prediction Accuracy Tests** (`test_services/models_medical/`).
...
- **Medical Tourism Whitepaper** (`Implementation Documentation/Destination_Based_Travelling Medicaltourism.pdf`).
- **MICE Event Planning Blueprint** (`Implementation Documentation/Destination_Based_Travelling-mice.pdf`).
- **Destination Wedding Planning Guide** (`Implementation Documentation/Destination_Based_Travelling-destination-wedding.pdf`).

---


### **🛠️ Project Setup Guide for Yatra-Vritta**  
...
## 🏥 4. Medical Tourism AI Model Setup
...
## 💍 6. Destination Wedding Module Setup
...
## ✈️ 7. Sightseeing & Flight Booking API Setup
...
### **For MongoDB**
If you’re using **MongoDB**, ensure you have a running instance and update `.env`:
...
## 🎯 11. Docker Setup (Optional for Deployment)
...
- **Event System on `http://localhost:3001`**
- **Wedding Planner on `http://localhost:3002`**

---

## 📌 12. Deployment Guide (Optional)
...
### **Deploy Event System on Railway**
```bash
cd event_management
railway up
```
...
```markdown
# 🧠 AI-Powered Ranking Engine for Smart Travel Recommendations
...
- **Weighted Score Computation** – Assigns importance to different ranking factors.
- **Min-Max Normalization** – Standardizes data across different metrics.
- **K-Nearest Neighbors (KNN) Imputation** – Handles missing data using AI.
- **Regression-Based Predictions** – Predicts missing values when necessary.
- **AI-Based Trend Forecasting** – Predicts emerging travel destinations.

---

## **1️⃣ Medical Tourism AI Ranking Model**

### **📌 Data Collection**
Medical tourism data is sourced from:
- **World Bank API** – Healthcare expenditure, hospital availability, GDP.
- **OECD Economic Indicators** – Business environment and stability scores.
- **United Nations World Tourism Organization (UNWTO)** – International tourism data.
...
```math
S_i = w_1 H_i + w_2 E_i + w_3 G_i + w_4 T_i + w_5 B_i + w_6 S_i
```
...
**Assigned Weights:**
```math
w_1 = 0.3, w_2 = 0.25, w_3 = 0.2, w_4 = 0.1, w_5 = 0.1, w_6 = 0.05
```
...
1. **K-Nearest Neighbors (KNN) Imputation**:
   ```math
   X^{(impute)}_{ij} = \frac{1}{k} \sum_{n \in N(i)} X_{nj}
   ```
2. **Regression-Based Predictions**:
   ```math
...
## **2️⃣ MICE Tourism AI Ranking Model**
### **📌 Data Collection**
- **World Bank API** – Business environment, ease of setting up operations.
- **International Air Transport Association (IATA)** – Global connectivity data.
- **UNWTO** – International arrivals and conference-related statistics.

### **📌 Weighted Score Formula**
```math
S_i = w_1 B_i + w_2 I_i + w_3 S_i + w_4 A_i + w_5 L_i
```
...
## **3️⃣ Destination Wedding AI Ranking Model**
### **📌 Data Collection**
- **World Bank API** – GDP, safety index, ease of business.
- **IATA** – International flight accessibility.
- **UNWTO** – Destination popularity and tourism demand.

### **📌 Weighted Score Formula**
```math
S_i = w_1 T_i + w_2 B_i + w_3 G_i + w_4 A_i + w_5 S_i
```
